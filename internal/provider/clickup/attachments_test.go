...
func TestProviderDownloadAttachment(t *testing.T) {
	tests := []struct {
		...
		{
			name:         "success: downloads attachment",
			workUnitID:   "task123",
			attachmentID: "https://api.clickup.com/api/v2/attachment/abc123",
			wantErr:      false,
		},
		{
			name:         "error: invalid attachment ID",
			workUnitID:   "task123",
		...
		},
		{
			...
			workUnitID:   "task123",
			attachmentID: "https://api.clickup.com/unauthorized",
			wantErr:      true,
		...
		},
		{
			...
			workUnitID:   "task123",
			attachmentID: "https://api.clickup.com/notfound",
			wantErr:      true,
			errContains:  "download failed",
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				// Check authorization (ClickUp uses custom token header, not Bearer)
				token := r.Header.Get("Authorization")
				if token != "test-token" {
					t.Errorf("expected Authorization header 'test-token', got %q", token)
				}
				...
				if tt.name == "error: invalid attachment ID" {
			...
			if tt.name == "success: downloads attachment" {
				attachmentID = server.URL
			}

			// Execute - attachmentID in ClickUp is the URL
