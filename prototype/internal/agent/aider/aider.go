...
// Agent wraps the aider CLI (https://github.com/paul-gauthier/aider)
type Agent struct {
	parser agent.Parser
	config agent.Config
}

// New creates an Aider agent with default config.
func New() *Agent {
	return &Agent{
		config: agent.Config{
...
// NewWithConfig creates an Aider agent with custom config.
...
// Available checks if the aider CLI is installed and configured.
func (a *Agent) Available() error {
	...
	if err := cmd.Run(); err != nil {
		return fmt.Errorf("aider CLI not working: %w", err)
	}
...
func (a *Agent) buildArgs(prompt string) []string {
	...
	// Non-interactive mode - automatically accept prompts
	args = append(args, "--yes")

	// Don't auto-commit - let Mehrhof handle git operations
	args = append(args, "--no-auto-commits")

	// Add the prompt via --message flag
...
// Register adds the Aider agent to a registry.
...
// PlainTextParser parses plain text output from aider.
type PlainTextParser struct{}

// NewPlainTextParser creates a plain text parser for aider output.
...
func (p *PlainTextParser) Parse(events []agent.Event) (*agent.Response, error) {
	...
	if fullText != "" {
		...
		response.Summary = summarizeAiderOutput(fullText)
	}

	return response, nil
}

// summarizeAiderOutput extracts a summary from aider's output.
func summarizeAiderOutput(text string) string {
	lines := strings.Split(text, "\n")
	if len(lines) == 0 {
		return ""
	}

	// Try to find meaningful summary lines
	// Aider often outputs status lines like "Applied edit to..." or "Commit..."
	...
	for _, line := range lines {
		...
		// Capture meaningful status lines
		if strings.HasPrefix(line, "Applied edit") ||
	...
	// Fall back to first non-empty line if no status lines found
	for _, line := range lines {
		line = strings.TrimSpace(line)
		if line != "" {
			if len(line) > 200 {
