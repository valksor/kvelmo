...
func TestRegistryDetect(t *testing.T) {
	...
	// Register a provider that matches
	factory := func(ctx context.Context, cfg Config) (any, error) {
		return &mockIdentifier{matchResult: true, parseResult: "parsed-id"}, nil
	}
	if err := r.Register(ProviderInfo{Name: "matcher", Priority: 10}, factory); err != nil {
		t.Fatalf("Register(matcher): %v", err)
	}

	ctx := context.Background()
	instance, id, err := r.Detect(ctx, "some-input", NewConfig())
	if err != nil {
		t.Fatalf("Detect failed: %v", err)
	}
	if id != "parsed-id" {
		t.Errorf("id = %q, want %q", id, "parsed-id")
	}
	if instance == nil {
...
func TestRegistryDetectNoMatch(t *testing.T) {
	...
	factory := func(ctx context.Context, cfg Config) (any, error) {
		return &mockIdentifier{matchResult: false}, nil
	}
	if err := r.Register(ProviderInfo{Name: "non-matcher"}, factory); err != nil {
		t.Fatalf("Register(non-matcher): %v", err)
	}

	ctx := context.Background()
	_, _, err := r.Detect(ctx, "some-input", NewConfig())
	if err == nil {
		t.Error("Detect should fail when no provider matches")
	}
}
...
func TestRegistryDetectPriority(t *testing.T) {
	...
	lowFactory := func(ctx context.Context, cfg Config) (any, error) {
		return &mockIdentifier{matchResult: true, parseResult: "low-id"}, nil
	}
	if err := r.Register(ProviderInfo{Name: "low", Priority: 1}, lowFactory); err != nil {
		t.Fatalf("Register(low): %v", err)
	}

	// Register high priority provider
	highFactory := func(ctx context.Context, cfg Config) (any, error) {
		return &mockIdentifier{matchResult: true, parseResult: "high-id"}, nil
	}
	if err := r.Register(ProviderInfo{Name: "high", Priority: 10}, highFactory); err != nil {
		t.Fatalf("Register(high): %v", err)
	}

	ctx := context.Background()
	_, id, err := r.Detect(ctx, "some-input", NewConfig())
	if err != nil {
		t.Fatalf("Detect failed: %v", err)
	}
	// Should use high priority provider
	if id != "high-id" {
		t.Errorf("id = %q, want %q (high priority)", id, "high-id")
	}
}
