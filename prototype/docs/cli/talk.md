...
```bash
mehr talk [message] [flags]
```

## Description

The `talk` command enters dialogue mode for:
...
Notes from talk sessions are saved and included in future planning and implementation prompts.
...
| `--agent-dialogue` |       | string |         | Override agent for dialogue       |
...
```bash
mehr talk "Use PostgreSQL for the database, not SQLite"
```
...
Resume the most recent talk session with full context.

### Continue Specific Session

```bash
mehr talk --session 2025-01-15T10-30-00-talk.yaml
```
...
```bash
mehr talk --agent-dialogue claude-opus
```
...
```bash
mehr start task.md
mehr talk "Focus on security - this will handle payments"
mehr plan
```
...
```bash
mehr plan
mehr talk "specification-2 is too complex. Can we simplify?"
...
# Not quite right...
mehr undo
mehr talk "Use dependency injection instead of global state"
mehr implement
```
...
```bash
mehr talk "The search should support fuzzy matching with typo tolerance"
```

### Ask Questions

```bash
mehr talk "What's the best way to handle rate limiting?"
```

### Provide Context

```bash
mehr talk "We're using the existing UserRepository for data access"
```

### Request Changes

```bash
mehr talk "Remove the caching - we'll add it later"
```
