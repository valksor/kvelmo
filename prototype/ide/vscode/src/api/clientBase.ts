...
export class MehrhofApiClientBase {
  ...
  protected get<T>(path: string, timeout?: number): Promise<T> {
    return this.request<T>('GET', path, undefined, timeout);
  }

  protected post<T>(path: string, body?: unknown, timeout?: number): Promise<T> {
