...
var memoryDownloadModelCmd = &cobra.Command{
	Use:   "download-model",
	Short: "Download the default ONNX embedding model",
	RunE:  runMemoryDownloadModel,
}

func init() {
	memoryDownloadModelCmd.Long = fmt.Sprintf(`Download the all-MiniLM-L6-v2 ONNX model (~23 MB) to
~/%s/models/.  The model is used for semantic search
and enables higher-quality memory retrieval than the TF-IDF fallback.

If the model is already cached this command returns immediately unless
--force is specified.`, meta.GlobalDir)
	MemoryCmd.AddCommand(memorySearchCmd)
	MemoryCmd.AddCommand(memoryStatsCmd)
	MemoryCmd.AddCommand(memoryClearCmd)
	MemoryCmd.AddCommand(memoryDownloadModelCmd)
	...
	memoryDownloadModelCmd.Flags().BoolP("force", "f", false, "Re-download even if the model is already cached")
}
...
func runMemoryDownloadModel(cmd *cobra.Command, args []string) error {
	globalPath := socket.GlobalSocketPath()
	if !socket.SocketExists(globalPath) {
		return errors.New("global socket not running\nRun '" + meta.Name + " serve' first")
	}

	client, err := socket.NewClient(globalPath, socket.WithTimeout(30*time.Minute))
	if err != nil {
		return fmt.Errorf("connect to global socket: %w", err)
	}
	defer func() { _ = client.Close() }()
	...
	fmt.Println("Downloading all-MiniLM-L6-v2 ONNX model (~23 MB)...")

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Minute)
	defer cancel()

	resp, err := client.Call(ctx, "memory.download_model", params)
	if err != nil {
		return fmt.Errorf("memory.download_model: %w", err)
	}
	...
	if result.Cached {
		fmt.Printf("Model already cached at: %s\n", result.ModelDir)
		fmt.Println("Use --force to re-download.")
	} else {
		fmt.Printf("Model downloaded successfully to: %s\n", result.ModelDir)
		fmt.Println("ONNX semantic embeddings will be active on next memory operation.")
		fmt.Println("")
		fmt.Println("Note: ONNX inference requires libonnxruntime to be installed.")
		fmt.Println("  Linux:  sudo apt install libonnxruntime-dev")
		fmt.Println("  macOS:  brew install onnxruntime")
		fmt.Println("  Custom: set ORT_DYLIB_PATH=/path/to/libonnxruntime.so")
		fmt.Println("  Without it, TF-IDF fallback remains active.")
	}
